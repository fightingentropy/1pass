...
import { decryptData, encryptData, InvalidPasswordError } from "@/lib/crypto";
import {
  fetchVaultStatus,
  initializeVault,
  loadVaultPayload,
...
export default function Home() {
  ...
  const unlocked = useMemo(
    () => vaultData !== null && sessionPassword !== null,
    [vaultData, sessionPassword],
  );
  ...
  const showUnlock = useMemo(
    () => vaultExists === true && !unlocked,
    [vaultExists, unlocked],
  );
  const shouldCenter = useMemo(
    () => showLoading || showSetup || showUnlock,
    [showLoading, showSetup, showUnlock],
  );
  ...
  useEffect(() => {
    ...
    for (const field of config.fields) {
      let value = dialogState.item
        ? ((dialogState.item as Record<string, string>)[field.key as string] ??
          "")
        : "";
  ...
  const checkVaultStatus = useCallback(async () => {
    ...
    try {
      ...
      console.error(error);
      setPageError(
        "Unable to reach vault storage. Check your connection and try again.",
      );
  ...
  const handleInitialize = useCallback(
    async (event: React.FormEvent<HTMLFormElement>) => {
      ...
      try {
        ...
        setFeedback({
          type: "success",
          message: "Vault initialized. Unlock it with your master password.",
        });
      } catch (error) {
        console.error(error);
        setFeedback({
  ...
  const unlockWithPassword = useCallback(async (password: string) => {
    ...
    const decrypted = await decryptData<VaultData>(payload, password);
  ...
  const persistVault = useCallback(
    async (data: VaultData, successMessage: string) => {
      ...
      try {
        ...
        await saveVaultPayload(payload);
        ...
        setFeedback({
          type: "error",
          message:
            error instanceof Error ? error.message : "Unable to save vault.",
        });
        return false;
      } finally {
        setIsSaving(false);
  ...
  const handleDialogSubmit = useCallback(
    async (event: React.FormEvent<HTMLFormElement>) => {
      ...
      const base: Record<string, string> =
        mode === "edit" && item ? { ...item } : { id: generateId() };
      ...
      const ok = await persistVault(
        updatedVault,
        mode === "edit" ? "Entry updated." : "Entry added.",
      );
      if (ok) {
        setDialogState(null);
      }
  ...
  const handleDelete = useCallback(
    async (category: VaultCategory, id: string) => {
      ...
      const updatedVault: VaultData = structuredClone(vaultData);
      updatedVault[category] = updatedVault[category].filter(
        (entry) => entry.id !== id,
      ) as never;
    ...
    return (
      <div className="text-sm text-muted-foreground rounded-2xl border border-dashed border-border/70 bg-background/80 p-8 text-center shadow-inner">
        ...
        <p className="mb-4 text-sm text-muted-foreground">
          Start by adding an entry – only encrypted data is stored in Cloudflare
          D1.
        </p>
        <Button
          variant="secondary"
  ...
  const renderPasswordEntry = useCallback(
    (entry: PasswordEntry) => (
      <Card
        key={entry.id}
        className="rounded-2xl border border-border/70 bg-background/70 shadow-sm backdrop-blur"
      >
        <CardHeader className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
          ...
          <CardAction className="flex flex-wrap items-center gap-2">
            ...
            <Button
              size="sm"
              variant="destructive"
              onClick={() => void handleDelete("passwords", entry.id)}
            >
              Delete
            </Button>
          </CardAction>
  ...
  const renderCardEntry = useCallback(
    (entry: CardEntry) => {
      const expiry = [entry.expiryMonth, entry.expiryYear]
        .filter(Boolean)
        .join("/");
          ...
          <CardHeader className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
            ...
            <CardAction className="flex flex-wrap items-center gap-2">
              ...
              <Button
                size="sm"
                variant="destructive"
                onClick={() => void handleDelete("cards", entry.id)}
              >
                Delete
              </Button>
            </CardAction>
  ...
  const renderIdentityEntry = useCallback(
    (entry: IdentityEntry) => (
      ...
      >
        <CardHeader className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
          ...
          <CardAction className="flex flex-wrap items-center gap-2">
            ...
            <Button
              size="sm"
              variant="destructive"
              onClick={() => void handleDelete("identities", entry.id)}
            >
              Delete
            </Button>
          </CardAction>
        ...
        <CardContent className="space-y-4">
          {entry.email ? (
            <DetailRow label="Email" value={entry.email} onCopy={handleCopy} />
          ) : null}
          {entry.phone ? (
            <DetailRow label="Phone" value={entry.phone} onCopy={handleCopy} />
          ) : null}
          {entry.nino ? (
            <DetailRow
              label="National Insurance Number"
          ...
          {entry.utr ? (
            <DetailRow label="UTR" value={entry.utr} onCopy={handleCopy} />
          ) : null}
          {entry.nhsNumber ? (
            <DetailRow
              label="NHS Number"
              value={entry.nhsNumber}
              onCopy={handleCopy}
            />
          ) : null}
          {entry.passportDetails ? (
            <DetailRow
              label="Passport Details"
              value={entry.passportDetails}
              multiline
            />
          ) : null}
          {entry.address ? (
            <DetailRow label="Address" value={entry.address} multiline />
          ) : null}
          {entry.notes ? (
            <DetailRow label="Notes" value={entry.notes} multiline />
          ) : null}
      ...
      >
        <header
          className={cn(
            ...
            shouldCenter
              ? "mx-auto max-w-sm items-center text-center sm:max-w-md"
        ...
        {showSetup ? (
          <Card className="w-full max-w-xl rounded-2xl border border-border/70 bg-background/80 shadow-lg backdrop-blur">
            <CardHeader className="space-y-1">
              ...
              <CardDescription>
                Pick a strong master password. It is never sent anywhere, so
                store it safely – you will need it to unlock your data.
              </CardDescription>
            </CardHeader>
            <CardContent>
        ...
        {unlocked && vaultData ? (
          <section className="flex flex-1 flex-col gap-8">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div className="space-y-1">
                ...
                <p className="text-sm text-muted-foreground">
                  Your encrypted data stays in Cloudflare D1; the master
            ...
            <Tabs
              value={activeTab}
              onValueChange={(value) => setActiveTab(value as VaultCategory)}
              className="flex-1"
            >
              <TabsList className="w-full overflow-x-auto rounded-full border border-border/60 bg-muted/50 p-1 text-sm shadow-sm sm:w-auto gap-1">
                <TabsTrigger
                  ...
                  onClick={() =>
                    startTransition(() => setActiveTab("passwords"))
                  }
                >
                  Passwords
                </TabsTrigger>
                ...
                <TabsTrigger
                  ...
                  className="rounded-full px-4 py-2 text-xs font-medium sm:px-6 sm:text-sm"
                  onClick={() =>
                    startTransition(() => setActiveTab("identities"))
                  }
                >
                  Identities
                </TabsTrigger>
              ...
              <TabsContent value="passwords" className="mt-6 space-y-5">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <p className="text-sm text-muted-foreground">
                    Credentials for logins and apps.
                  </p>
                  <Button
                    size="sm"
                    className="rounded-full px-4"
                    onClick={() =>
                      setDialogState({ category: "passwords", mode: "create" })
                    }
                  >
                    Add password
                  </Button>
                ...
                {vaultData.passwords.length === 0 ? (
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    {vaultData.passwords.map((entry) =>
                      renderPasswordEntry(entry),
                    )}
                  </div>
                )}
              </TabsContent>
              <TabsContent value="cards" className="mt-6 space-y-5">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <p className="text-sm text-muted-foreground">
                    Payment cards stay encrypted at rest.
                  </p>
                  <Button
                    size="sm"
                    className="rounded-full px-4"
                    onClick={() =>
                      setDialogState({ category: "cards", mode: "create" })
                    }
                  >
                    Add card
                  </Button>
              ...
              <TabsContent value="identities" className="mt-6 space-y-5">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <p className="text-sm text-muted-foreground">
                    Profiles and address information.
                  </p>
                  <Button
                    size="sm"
                    className="rounded-full px-4"
                    onClick={() =>
                      setDialogState({ category: "identities", mode: "create" })
                    }
                  >
                    Add identity
                  </Button>
                ...
                {vaultData.identities.length === 0 ? (
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    {vaultData.identities.map((entry) =>
                      renderIdentityEntry(entry),
                    )}
                  </div>
                )}
              </TabsContent>
                ...
                <div className="space-y-4">
                  {CATEGORY_CONFIG[dialogState.category].fields.map((field) => (
                    <div key={String(field.key)} className="space-y-2">
                      <Label htmlFor={`field-${String(field.key)}`}>
                        {field.label}
                      </Label>
                      {field.multiline ? (
                        <Textarea
  ...
  return (
    <div className="flex flex-col gap-1 rounded-xl border border-border/60 bg-background/80 p-4 text-sm shadow-inner">
      ...
      <div
        className={cn(
          "text-foreground",
          multiline ? "whitespace-pre-line" : "truncate font-medium",
        )}
      >
        {displayValue}
      </div>
